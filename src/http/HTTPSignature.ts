...
export default class HTTPSignature {
  ...
  public getParams(): object {
    ...
    if (!this.request.headers['authorization']) throw new Error('no authorization header in the request');
    ...
    if (method.toLowerCase() !== 'signature') throw new Error('authorization schema is not "Signature"');

    const regex = /(\w+)s*=s*"([^"]+)"s*(,|$)/g;
    let match;
    this.params = {};
    while ((match = regex.exec(paramString))) this.params[match[1]] = match[2];
  ...
  public getMessage(): string {
    return this.getHeaders()
      .map((header) => {
        if (header === '(request-target)') return `(request-target): ${this.request.getRequestTarget()}`;
        else return `${header}: ${this.request.headers[header]}`;
      })
      .join('\n');
  }
  ...
  public verify(): void {
    ...
    if (!crypto.verifySignature(bytes, Binary.fromBase64(signature))) throw new Error('invalid signature');
  ...
  protected assertParams(): void {
    ...
    required.forEach((param) => {
      if (!(param in this.params)) throw new Error(`${param} was not specified`);
    });
