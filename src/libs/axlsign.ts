...
function ed25519_sign(sm?: any, m?: any, n?: any, edsk?: any, opt_rnd?: any) {
...
//     // otherwise it must have n + 64 bytes.
//
//     const edsk = convertPrivateKey(sk);
//
//     // Remember sign bit.
//     const signBit = edsk[63] & 128;
//     let smlen;
...
function convertPrivateKey(sk: any) {
...
function ed25519_sign_open(m, sm, n, edpk) {
...
axlsign.signMessage = function(secretKey, msg, opt_random) {
    ...
    if (opt_random) {
        ...
        ed25519_sign(buf, msg, msg.length, secretKey, opt_random);
        return new Uint8Array(buf.subarray(0, 64 + msg.length));
    } else {
        const signedMsg = new Uint8Array(64 + msg.length);
        ed25519_sign(signedMsg, msg, msg.length, secretKey);
        return signedMsg;
    }
};
...
axlsign.openMessage = function(publicKey, signedMsg) {
    ...
    const mlen = ed25519_sign_open(tmp, signedMsg, signedMsg.length, publicKey);
...
axlsign.sign = function(secretKey, msg, opt_random) {
    ...
    if (secretKey.length !== 64) throw new Error('wrong secret key length');
    if (opt_random) {
        checkArrayTypes(opt_random);
        if (opt_random.length !== 64) throw new Error('wrong random data length');
    }
