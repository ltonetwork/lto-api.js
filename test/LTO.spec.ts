...
describe('LTO', () => {
  ...
  describe('#signRequest', () => {
    it('should create a correct signature on get requests', () => {
      ...
      expect(signature).to.eq('keyId="GuCK3Vaemyc3fUH94WUZ8tdQUZuG6YQmQBh93mu8E67F",algorithm="ed25519-sha256",headers="(request-target) date",signature="K1FrH2F2Y/pAbYj2R3EV6JXaL+6HQXB57g2xAsNZeUH0ofhz1dDbBYcNm/r9LKHHLaHMYiW0qrF6mbTUxv6lDg=="');
    });
    ...
    it('should create a correct signature on get requests without sha256', () => {
      ...
      expect(signature).to.eq('keyId="GuCK3Vaemyc3fUH94WUZ8tdQUZuG6YQmQBh93mu8E67F",algorithm="ed25519",headers="(request-target) date",signature="5c/H1bpfsr0OQ2VQDgUwUEDKX4KZe+Gjo2Z4xhy+3ZXs6qBadaUclnRKsvvc0tfZuXVJ3rY8BiuKnEL8Ae78Cw=="');
    });
    ...
    it('should create a correct signature on post with digest', () => {
      ...
      const seed = lto.seedFromExistingPhrase(phrase);
      const signature = lto.signRequest(headers, seed.signKeys.publicKey, seed.signKeys.privateKey, false);
      expect(signature).to.eq('keyId="GuCK3Vaemyc3fUH94WUZ8tdQUZuG6YQmQBh93mu8E67F",algorithm="ed25519-sha256",headers="(request-target) date digest",signature="djy8HhFW7pt6852KKYVTGVBxlpX1qI3fUIRaKmtbmqEd0as8XbxGU4sGQ4Pf6mN36APyTCMu7w65LSM2/BMTAg=="');
    });
    ...
    it('should create a correct signature on post with digest and content-length', () => {
      ...
      const seed = lto.seedFromExistingPhrase(phrase);
      const signature = lto.signRequest(headers, seed.signKeys.publicKey, seed.signKeys.privateKey, false);
      expect(signature).to.eq('keyId="GuCK3Vaemyc3fUH94WUZ8tdQUZuG6YQmQBh93mu8E67F",algorithm="ed25519-sha256",headers="(request-target) date digest content-length",signature="Ay7+4WXyTONMW2WsIzUlj2DM7FHALPHZc54yIpE9/lia69cs9gaBwBVFMJU0AtGAiozUYK7e3UrRclSs0ueADg=="');
    });
  ...
  describe('#verifyRequest', () => {
    it('should verify a request', () => {
      ...
      const headers = {
        ...
        'content-length': 8192,
        signature: 'keyId="9MTLMUaePzjm2GL3J6Zxv9ersEa6bng7b8rVAeAft8wu",algorithm="ed25519-sha256",headers="(request-target) x-date digest content-length",signature="/iy+odu8JxOm9YIWYmFQP8KjhBpWYpQLOeV1psC+ISk3gePdLKGEcC11SZwhYaJw381XFEp13Pg29rwc8reUAg=="'
      };
